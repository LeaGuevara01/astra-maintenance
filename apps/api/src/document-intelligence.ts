...
export function normalizeInventory(records: SourceRecord[], previous: SourceRecord[] = []): NormalizedInventory {
  ...
  const sources: NormalizedSource[] = records.map((record, index) => {
    ...
    const validPage = (page: number) => Number.isInteger(page) && page > 0 && (totalPages === null || page <= totalPages);
    const pageEvidenceValid = record.pagesNeedingOCR !== undefined && record.pagesNeedingOCR.every(validPage);
    const pagesNeedingOCR = [...new Set((record.pagesNeedingOCR ?? []).filter(validPage))].sort((a, b) => a - b);
    ...
    // Missing page evidence cannot establish complete text extraction.
    const coverageKnown = totalPages !== null && pageEvidenceValid
      && ['TEXT_EXTRACTED', 'PARTIAL', 'OCR_REQUIRED'].includes(status)
      && (status === 'TEXT_EXTRACTED' || pagesNeedingOCR.length > 0);
    const pagesWithText = coverageKnown ? totalPages! - pagesNeedingOCR.length : null;
    ...
    if (pagesWithText === null) warnings.push('COBERTURA_DESCONOCIDA');
    ...
    return { ...record, id, name: record.name ?? path.basename(record.path), ext: record.ext ?? path.extname(record.path).toLowerCase(), kind: record.kind ?? 'UNCLASSIFIED', extractionStatus: duplicateOf && duplicateOf !== id ? 'DUPLICATE' : status, reviewStatus: 'PENDING', reviewDecision: 'A_CONFIRMAR', coverage: { totalPages, pagesWithText, pagesNeedingOCR, percent: pagesWithText === null || totalPages === null ? null : Math.round((pagesWithText / totalPages) * 100), partial: status === 'PARTIAL' }, warnings: duplicateOf && duplicateOf !== id ? [...warnings, `DUPLICADO_DE: ${duplicateOf}`] : warnings, ...(duplicateOf && duplicateOf !== id ? { duplicateOf } : {}), revisionStatus };
  });
  const ocrQueue = sources.filter(source => source.extractionStatus === 'OCR_REQUIRED' || source.extractionStatus === 'PARTIAL').map(source => ({ sourceId: source.id, path: source.path, pages: source.coverage.pagesNeedingOCR, reason: source.coverage.percent === null ? 'Cobertura desconocida; determinar páginas que requieren OCR.' : source.extractionStatus === 'PARTIAL' ? 'Páginas sin texto en una fuente parcialmente extraída.' : 'La fuente no tiene texto extraíble.', status: 'PENDING' as const }));
