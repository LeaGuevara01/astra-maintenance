...
export async function renderCardPdf(order: OrderView, format: 'a6' | 'a4', origin: string): Promise<Buffer> {
  ...
  let fontSize = 7.7;
  const measure = () => card.rows.reduce((sum, row) => { doc.font(row.heading ? 'Helvetica-Bold' : 'Helvetica').fontSize(row.heading ? fontSize + 0.5 : fontSize); return sum + doc.heightOfString(row.text, { width: innerWidth, lineGap: 0.5 }) + (row.heading ? 6 : 3); }, 0);
  while (measure() > cardHeight - 102 && fontSize > 6.6) fontSize -= 0.2;
  assert(measure() <= cardHeight - 102, 422, 'CARD_CAPACITY', 'El detalle excede la capacidad legible A6. Consulte el registro digital; reduzca el alcance de la próxima OT.');
